...
@Service
public class GetClanCoinDiscord implements DiscordEventListener<MessageCreateEvent> {
    private final GoogleSheetsService googleSheetsService;

    private static final String COMMAND = "!misMonedas";

    @Autowired
    public GetClanCoinDiscord(final GoogleSheetsService googleSheetsService) {
    ...
    private String getMessage(final MessageCreateEvent event) {
        return "Actualmente "
                + getUserId(event)
                + " tiene "
                + this.googleSheetsService.getMyClanCoins(getUserName(event))
                + " monedas de clan";
    }
